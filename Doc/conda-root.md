...
5. Verificación de la instalación de conda
    ...
    conda --version
    ```
6. Instalación de ROOT con Conda
    ```bash
    conda config --set channel_priority strict
    conda create -n lhcb-root -c conda-forge root xrootd python=3.11
    ```
    ROOT está disponible desde `conda-forge`, este paquete permite usar tanto `root` desde terminal como `import ROOT` desde Python.
...
9. En caso de que aparezca 
    ...
    conda activate lhcb-root
    ```
10. Verificación de la instalación de PyROOT
    ```bash
    python -c "import ROOT; print (ROOT.gROOT.GetVersion())"
    ```
    <[Regresar](/PF/README.md)>